...
package de.jpaw.bonaparte.jpa.dsl.scoping;
...
import de.jpaw.bonaparte.jpa.dsl.bDDL.ForeignKeyDefinition;
...
import de.jpaw.bonaparte.jpa.dsl.bDDL.SubGraphFields;
 ...
 *  */
public class BDDLScopeProvider1 extends ImportedNamespaceAwareLocalScopeProvider { // was autogenerated: AbstractDeclarativeScopeProvider {
    // private static final Logger LOGGER = Logger.getLogger(BDDLScopeProvider1.class);

    private void recursivelyAddColumnsOfClassAndParents(List<ImportNormalizer> preliminaryResult, ClassDefinition cl, boolean ignoreCase) {
        while (cl != null) {
    ...
    // select possible references for NamedEntityGraph subgraphs
    private List<ImportNormalizer> getRelationsSub(EntityDefinition entity, boolean ignoreCase) {
        List<ImportNormalizer> preliminaryResult = new ArrayList<ImportNormalizer>(10);
        BDDLPackageDefinition pkg = (BDDLPackageDefinition)entity.eContainer();
        String qualifiedImportNamespace = pkg.getName() + "." + entity.getName() + ".*";
//        LOGGER.info("ScopeProvider called for " + qualifiedImportNamespace);
//        System.out.println("ScopeProvider called for " + qualifiedImportNamespace + " (stdout)");
        preliminaryResult.add(createImportedNamespaceResolver(qualifiedImportNamespace, ignoreCase));
        return preliminaryResult;
    }
    ...
    protected List<ImportNormalizer> internalGetImportedNamespaceResolvers(EObject context, boolean ignoreCase) {
        if ((context instanceof SingleColumn ||
            ...
            return getColumnsSub(YUtil.getBaseEntity(context.eContainer()), ignoreCase);
        } else if (context instanceof ListOfColumns) {
            if (context.eContainer() instanceof Relationship && context.eContainer().eContainer() instanceof OneToMany) {
                Relationship er = (Relationship)context.eContainer();
                return getColumnsSub(er.getChildObject(), ignoreCase);
            } else {
                return getColumnsSub(YUtil.getBaseEntity(context.eContainer()), ignoreCase);
            }
//        } else if (context instanceof NamedEntityGraph) {
//            System.out.println("ScopeProvider for NamedEntityGraph referenced is " + ((NamedEntityGraph)context).getName() + ", container is " + context.eContainer().getClass().getSimpleName());
//            return super.internalGetImportedNamespaceResolvers(context, ignoreCase);
//        } else if (context instanceof GraphRelationship) {
//            System.out.println("ScopeProvider for GraphRelationship referenced is " + ((GraphRelationship)context).getName() + ", container is " + context.eContainer().getClass().getSimpleName());
//            return super.internalGetImportedNamespaceResolvers(context, ignoreCase);
        } else if (context instanceof SubGraphFields) {
            // referencing relations in a different entity
            Relationship referenced = ((GraphRelationship)(context.eContainer())).getName();
//            LOGGER.info("ScopeProvider referenced is " + referenced.getName());
//            System.out.println("ScopeProvider referenced is " + referenced.getName() + " (stdout)");
            return getRelationsSub(referenced.getChildObject(), ignoreCase);
